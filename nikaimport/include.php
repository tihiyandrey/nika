...
class CIBlockCMLImportCustom extends \CIBlockCMLImport {
    ...
    var $_xml_file                 = null; /** @var CIBlockXMLFile $_xml_file */
    var $bCatalog                  = false;
        ...
        if (is_array($file)) {
            ...
            if (strlen($file) > 0) {
                ...
                if ($fileId > 0) {
                    ...
                    if (is_array($tmp_ar_file = CFile::MakeFilearray($file))) {
                        return $tmp_ar_file;
                    }
                }
            }
    ...
    function ResizePicture($file, $resize, $primaryField, $secondaryField = "") {
        ...
        $tempFile  = $this->files_dir . "img/" . bx_basename($file);
        ...
        $Image = file_get_contents($file, false, stream_context_create($arContext));
        if (file_put_contents($tempFile, $Image)) {
            if (is_array($resize)) {
                $width    = intval($resize[0]) > 0 ? $resize[0] : 500;
                $height   = intval($resize[1]) > 0 ? $resize[1] : 500;
                $new_file = preg_replace("#(\\.)([^./\\\\]+?)$#", ".resize.$width.$height.\\2", $tempFile);
                CFile::ResizeImageFile($tempFile, $new_file, array("width" => $width, "height" => $height));
                return CFile::MakeFilearray($new_file, false, false, $external_id);
            } else {
                return CFile::MakeFilearray($tempFile, false, false, $external_id);
            }
        }
        return $errorFile;
    }
    ...
    function CheckProperty($IBLOCK_ID, $code, $xml_name) {
        ...
        if (!$dbProperty) {
            ...
            if (!$ID) {
                AddMessage2Log($obProperty->LAST_ERROR, "import_nika");
    ...
    function CheckCurrency($currency) {
        ...
        if ($currency == $this->mess["IBLOCK_XML2_RUB"]) {
            ...
            if (is_array($CML2_CURRENCY) && isset($CML2_CURRENCY[$currency]) && is_string($CML2_CURRENCY[$currency]) && preg_match("/^[a-zA-Z0-9]+$/", $CML2_CURRENCY[$currency])) {
                ...
                AddMessage2Log($this->LAST_ERROR, "import_nika");
            }
        }
    ...
    function ImportMetaData($xml_root_id, $IBLOCK_TYPE, $IBLOCK_LID, $bUpdateIBlock = true) {
        ...
        if ($ar) {
            foreach(array(LANGUAGE_ID, "en", "ru") as $lang) {
                $mess = IncludeModuleLangFile(__FILE__, $lang, true);
                if ($ar["NAME"] === $mess["IBLOCK_XML2_COMMERCE_INFO"]) { // КоммерческаяИнформация
                    $this->mess              = $mess;
        ...
        $rs = $this->_xml_file->GetList(
            ...
            array(
                ...
                "NAME"      => $this->mess["IBLOCK_XML2_CATALOG"] // Каталог
            ),
            array(
                "ID",
                "ATTRIBUTES"
        ...
        if (!$ar) {
            $rs = $this->_xml_file->GetList(
                ...
                array(
                    "PARENT_ID" => $xml_root_id,
                    "NAME"      => $this->mess["IBLOCK_XML2_OFFER_LIST"] // ПакетПредложений
                ),
                array(
                    "ID",
                    "ATTRIBUTES"
        ...
        if (!$ar) {
            $rs = $this->_xml_file->GetList(
                ...
                array(
                    "PARENT_ID" => $xml_root_id,
                    "NAME"      => $this->mess["IBLOCK_XML2_OFFERS_CHANGE"] // ИзмененияПакетаПредложений
                ),
                array(
                    "ID",
                    "ATTRIBUTES"
        ...
        if ($ar) {
            ...
            while ($ar = $rs->Fetch()) {
                ...
                if ($ar["NAME"] == $this->mess["IBLOCK_XML2_ID"]) { // Ид
                    $arIBlock["XML_ID"] = ($this->use_iblock_type_id ? $IBLOCK_TYPE . "-": "") . $ar["VALUE"];
                    if (isset($_SESSION["NIKAIMPORT"]["PARAMS"]["XML_ID"])) {
                        $arIBlock["XML_ID"] = $_SESSION["NIKAIMPORT"]["PARAMS"]["XML_ID"];
                    }
                }
                elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_CATALOG_ID"]) { // ИдКаталога
                    $arIBlock["CATALOG_XML_ID"] = ($this->use_iblock_type_id? $IBLOCK_TYPE."-": "").$ar["VALUE"];
                    if (isset($_SESSION["NIKAIMPORT"]["PARAMS"]["XML_ID"])) {
                        $arIBlock["CATALOG_XML_ID"] = $_SESSION["NIKAIMPORT"]["PARAMS"]["XML_ID"];
                    }
                }
                ...
                elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_POSITIONS"] || $ar["NAME"] == $this->mess["IBLOCK_XML2_OFFERS"]) // Товары || Предложения
                ...
                elseif($ar["NAME"] == $this->mess["IBLOCK_XML2_METADATA_ID"]) // ИдКлассификатора
        ...
        if($meta_data_xml_id)
        {
            ...
            while($arMetadata = $rs->Fetch())
            {
                ...
                if($bMetaFound)
                {
                    ...
                    {
                        if($arMeta["NAME"] == $this->mess["IBLOCK_XML2_GROUPS"]) // Группы
    ...
    function ImportSectionsCustom($start_time, $interval) {
        $i = 0;
        if (!isset($_SESSION["NIKAIMPORT"]["SECTIONS_AR"])) {
            $arParent = array();
            $rs = $this->_xml_file->GetList(
                array(
                    "ID" => "asc"
                ...
                array(
                    "NAME" => $this->mess["IBLOCK_XML2_GROUPS"] // Группы
                ),
                array()
            );
            while ($ar = $rs->Fetch()) {
                $arParent[$ar["ID"]] = $ar["PARENT_ID"];
            }
            unset($ar, $rs);
            $_SESSION["NIKAIMPORT"]["SECTIONS_AR"] = $arParent;
            $_SESSION["NIKAIMPORT"]["SECTIONS_IMPORT_LAST"] = 0;
        }
        $rs = $this->_xml_file->GetList(
            array(
                "ID" => "asc"
            ...
            array(
                "NAME" => $this->mess["IBLOCK_XML2_GROUP"],
                ">ID"  => $_SESSION["NIKAIMPORT"]["SECTIONS_IMPORT_LAST"]
            ),
            array()
        );
        while ($ar = $rs->Fetch()) {
            $arParent  = $_SESSION["NIKAIMPORT"]["SECTIONS_AR"];
            $parent_id = false;
            /**
             * поиск родителя
             */
            if (isset($arParent[$ar["PARENT_ID"]])) {
                $rsParent = $this->_xml_file->GetList(
                    array(),
                    array(
                        "NAME"      => $this->mess["IBLOCK_XML2_ID"],
                        "PARENT_ID" => $arParent[$ar["PARENT_ID"]]
                    ),
                    array(
                        "VALUE"
                    )
                );
                if ($arParentRes = $rsParent->Fetch()) {
                    $resSection = \Bitrix\Iblock\SectionTable::getList(
                        array(
                            "filter" => array(
                                "IBLOCK_ID" => $_SESSION["NIKAIMPORT"]["PARAMS"]["IBLOCK_ID_CATALOG"],
                                "XML_ID"    => $arParentRes["VALUE"]
                            ),
                            "select" => array(
                                "ID"
                            ),
            ...
            $result = $this->ImportSection($ar["ID"], $_SESSION["NIKAIMPORT"]["PARAMS"]["IBLOCK_ID_CATALOG"], $parent_id);
            $i++;
            $_SESSION["NIKAIMPORT"]["SECTIONS_IMPORT_LAST"] = $ar["ID"];
    ...
    function ImportPrices($XML_PRICES_PARENT, $IBLOCK_ID, $IBLOCK_LID) {
        ...
        foreach ($arXMLPrices as $arXMLPrice) {
            ...
            } elseif (strlen($arPrices[$found_id]["XML_ID"]) <= 0 && strlen($PRICE_ID) >= 0) { // We can update XML_ID of the price
            ...
            } elseif ($arPrices[$found_id]["NAME"] !== $PRICE_NAME && !$this->disable_change_price_name) { // We should update NAME of the price
                CCatalogGroup::Update(
                    $found_id,
                    array(
                        "NAME" => $PRICE_NAME,
    ...
    function ImportBaseUnits($XML_BASE_UNITS_PARENT) {
        ...
        foreach ($arXMLBaseUnits as $arXMLBaseUnit) {
            ...
            $rsBaseUnit = CCatalogMeasure::GetList(
                array(),
                array(
                    "CODE" => $arUnit["CODE"]
                )
        ...
        foreach ($arXMLStores as $arXMLStore) {
            ...
            if (isset($arXMLStore[$this->mess["IBLOCK_XML2_STORE_CONTACTS"]]) && is_array($arXMLStore[$this->mess["IBLOCK_XML2_STORE_CONTACTS"]])) {
                ...
                foreach ($arXMLStore[$this->mess["IBLOCK_XML2_STORE_CONTACTS"]] as $arContact) {
                    if (is_array($arContact) && (!isset($arContact[$this->mess["IBLOCK_XML2_TYPE"]]) || substr($arContact[$this->mess["IBLOCK_XML2_TYPE"]], 0, strlen($this->mess["IBLOCK_XML2_STORE_PHONE"])) === $this->mess["IBLOCK_XML2_STORE_PHONE"])) {
    ...
    function ImportProperties($XML_PROPERTIES_PARENT, $IBLOCK_ID) {
        ...
        $rs = $this->_xml_file->GetList(
            ...
            array(
                "PARENT_ID" => $XML_PROPERTIES_PARENT
            ),
            array(
                "ID"
            )
        ...
        while ($ar = $rs->Fetch()) {
            ...
            while ($arP = $rsP->Fetch()) {
                ...
                } elseif ($arP["NAME"]==$this->mess["IBLOCK_XML2_VALUES_TYPES"]) { // This property metadata contains information about it's type
    ...
    function ImportSectionProperties($XML_SECTION_PROPERTIES, $IBLOCK_ID, $SECTION_ID = 0) {
        ...
        $rs = $this->_xml_file->GetList(
            ...
            array(
                "PARENT_ID" => $XML_SECTION_PROPERTIES
            ),
            array(
                "ID"
            )
        ...
        if (!is_array($PRICES_MAP)) {
            ...
            if (isset($this->next_step["XML_PRICES_PARENT"])) {
                $rs = $this->_xml_file->GetList(
                    ...
                    array(
                        "PARENT_ID" => $this->next_step["XML_PRICES_PARENT"]
                    )
                );
    ...
    function ImportElements($start_time, $interval)
    {
        ...
        if ($this->next_step["XML_ELEMENTS_PARENT"]) {
            ...
            while ($arParent = $rsParents->Fetch()) {
                ...
                } elseif(array_key_exists($this->mess["IBLOCK_XML2_BX_TAGS"], $arXMLElement)) { // БитриксТеги
                    $ID = $this->ImportElement($arXMLElement, $counter, $bWF, $arParent);
                } else {
                    $this->arFileDescriptionsMap = array();
                    $this->arElementFilesId      = array();
                    $this->arElementFiles        = array();
                    if ($this->next_step["bOffer"]) { // offers.xml
                        if (!$this->use_offers) { // We have only one information block
                            $ID = $this->ImportElementPrices($arXMLElement, $counter, $arParent);
                        } elseif ($hashPosition === false && !$this->force_offers) { // We have separate offers iblock and there is element price
                            $ID = $this->ImportElementPrices($arXMLElement, $counter, $arParent);
                        } else {
                            $xmlKeys = array_keys($arXMLElement);
                            if ($xmlKeys == array($this->mess["IBLOCK_XML2_ID"], $this->mess["IBLOCK_XML2_PRICES"])) { // prices.xml
                                $ID = $this->ImportElementPrices($arXMLElement, $counter, $arParent);
                            } elseif ($xmlKeys == array($this->mess["IBLOCK_XML2_ID"], $this->mess["IBLOCK_XML2_RESTS"])) { // rests.xml
                                $ID = $this->ImportElementPrices($arXMLElement, $counter, $arParent);
                            } else { // It's an offer in offers iblock
                                $ID = $this->ImportElement($arXMLElement, $counter, $bWF, $arParent);
                            }
                        }
                    } else { // import.xml
    ...
    function CheckIfElementIsActive($arXMLElement) {
        $bActive = true;
        if (isset($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]])) { // ЗначенияСвойств
            foreach ($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]] as $value) {
                if ($value[$this->mess["IBLOCK_XML2_ID"]] === "CML2_ACTIVE") {
                    if ($value[$this->mess["IBLOCK_XML2_VALUE"]] === "false" || $value[$this->mess["IBLOCK_XML2_VALUE"]] === "0") { // Значение
                        $bActive = false;
                        break;
                    }
                }
    ...
    function ImportElement($arXMLElement, &$counter, $bWF, $arParent) {
        ...
        $bMatch = false;
        $hide_catalog = $arXMLElement[$this->mess["IBLOCK_XML2_ACTIVE"]] == "N" ? $this->mess["YES"] : false;
        if (!$hide_catalog && isset($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]])) {
            foreach ($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]] as $value) {
                ...
                if ($prop_id == 700) {
                    $hide_catalog = !empty($value[$this->mess["IBLOCK_XML2_VALUE"]]) ? $this->mess["YES"] : false;
                    break;
                }
            }
        }
        if ($arDBElement = $rsElement->Fetch()) {
            ...
            if (!$this->use_crc) {
                /**
                 * св-во MORE_PHOTO удаляем картинки
                 */
                $more_photo_ob = $obElement->GetProperty(
                ...
                $arFile = array();
                while ($more_photo = $more_photo_ob->Fetch()) {
                    $arFile[$more_photo["PROPERTY_VALUE_ID"]]["VALUE"] = array("del" => "Y");
                }
                $obElement->Update(
                    $arDBElement["ID"],
                    array(
        ...
        if ($bMatch && $this->use_crc) {
            CIBlockElement::SetPropertyValuesEx(
                ...
                $arDBElement["IBLOCK_ID"],
                array(
                    "SKRYT_V_SPISKE_KATALOGA" => $hide_catalog
                )
            );
            $counter["UPD"]++;
            ...
            if (array_key_exists($this->mess["IBLOCK_XML2_MANUFACTURER"], $arXMLElement) && $this->PROPERTY_MAP["CML2_MANUFACTURER"] > 0) {
            ...
            if (array_key_exists($this->mess["IBLOCK_XML2_FILE"], $arXMLElement) && strlen($this->PROPERTY_MAP["CML2_FILES"]) > 0) { // Файл
            ...
            if (isset($arXMLElement[$this->mess["IBLOCK_XML2_GROUPS"]])) { // Группы
            ...
            if (array_key_exists($this->mess["IBLOCK_XML2_AMOUNT"], $arXMLElement)) { // Количество
                $arElementTmp = array();
            ...
            if (isset($arXMLElement[$this->mess["IBLOCK_XML2_ITEM_ATTRIBUTES"]])) { // ХарактеристикиТовара
            ...
            $i = 0;
            $weightKey = false;
            if (isset($arXMLElement[$this->mess["IBLOCK_XML2_TRAITS_VALUES"]])) { // ЗначенияРеквизитов
                ...
                foreach ($arXMLElement[$this->mess["IBLOCK_XML2_TRAITS_VALUES"]] as $value) {
                    if (!array_key_exists("PREVIEW_TEXT", $arElement) && $value[$this->mess["IBLOCK_XML2_NAME"]] == $this->mess["IBLOCK_XML2_FULL_TITLE2"]) {
                    ...
                    } elseif ($value[$this->mess["IBLOCK_XML2_NAME"]] == $this->mess["IBLOCK_XML2_HTML_DESCRIPTION"]) {
                        if (strlen($value[$this->mess["IBLOCK_XML2_VALUE"]]) > 0) {
                            $arElement["DETAIL_TEXT"] = $value[$this->mess["IBLOCK_XML2_VALUE"]];
                            $arElement["DETAIL_TEXT_TYPE"] = "html";
                        }
                    } elseif ($value[$this->mess["IBLOCK_XML2_NAME"]] == $this->mess["IBLOCK_XML2_FILE"]) {
                        if (strlen($value[$this->mess["IBLOCK_XML2_VALUE"]]) > 0) {
                            $prop_id = $this->PROPERTY_MAP["CML2_FILES"];
                            $j = 1;
                    ...
                    } elseif ($value[$this->mess["IBLOCK_XML2_NAME"]] == $this->mess["IBLOCK_XML2_FILE_DESCRIPTION"]) {
                        if (strlen($value[$this->mess["IBLOCK_XML2_VALUE"]]) > 0) {
            ...
            if (isset($arXMLElement[$this->mess["IBLOCK_XML2_WEIGHT"]])) { // Вес
                if ($weightKey !== false) {
            ...
            if (isset($arXMLElement[$this->mess["IBLOCK_XML2_TAXES_VALUES"]])) { // СтавкиНалогов
            ...
            while ($arBaseUnit = $rsBaseUnit->Fetch()) {
                if (strlen($arBaseUnit["ATTRIBUTES"]) > 0) {
                    ...
                    if (is_array($info) && array_key_exists($this->mess["IBLOCK_XML2_CODE"], $info)) {
                        $arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]] = $info[$this->mess["IBLOCK_XML2_CODE"]];
                    }
                }
            }
            if (isset($arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]])) { // БазоваяЕдиница
            ...
            if (isset($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]])) { // ЗначенияСвойств
                foreach ($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]] as $value) {
                    if (!array_key_exists($this->mess["IBLOCK_XML2_ID"], $value)) { // Ид
                        continue;
                    }
                    $prop_id = $value[$this->mess["IBLOCK_XML2_ID"]];
                    ...
                    if (array_key_exists($this->mess["IBLOCK_XML2_PROPERTY_VALUE"], $value)) { // ЗначениеСвойства
                        $i     = 1;
                        ...
                        foreach ($value as $k => $prop_value) {
                            ...
                            if ($prop_id_xml == "CML2_PICTURES") {
                                if (is_array($this->detail)) {
                                    $prop_value = $this->ResizePicture($prop_value, $this->detail);
                                } else {
                                    $prop_value  = $this->MakeFilearray($prop_value);
                                }
                            }
                            if ($prop_id_xml == 700) {
                                $prop_value = $hide_catalog;
                            }
                            $arElement["PROPERTY_VALUES"][$prop_id]["n".$i] = array(
                                "VALUE"       => $prop_value,
                                "DESCRIPTION" => false,
            ...
            /**
             * св-во что товар импортирован с Ника
             * 422 св-вa
             */
            $arElement["PROPERTY_VALUES"][422] = array(
                "n0" => array(
                    "VALUE"       => $this->mess["YES"],
                    "DESCRIPTION" => "Метка товара Ника"
                )
            );
            if ($arDBElement) {
                 ...
                 * add
                 */
                $arElement["ID"] = $obElement->Add($arElement, false, true, $this->iblock_resize);
        ...
        } elseif (array_key_exists($this->mess["IBLOCK_XML2_PRICES"], $arXMLElement)) { //Collect price information for future use
            $arElement["PRICES"] = array();
            if (is_array($arXMLElement[$this->mess["IBLOCK_XML2_PRICES"]])) {
                foreach ($arXMLElement[$this->mess["IBLOCK_XML2_PRICES"]] as $price) {
                    if (isset($price[$this->mess["IBLOCK_XML2_PRICE_TYPE_ID"]]) && array_key_exists($price[$this->mess["IBLOCK_XML2_PRICE_TYPE_ID"]], $this->PRICES_MAP)) {
        ...
        } elseif (array_key_exists($this->mess["IBLOCK_XML2_STORES"], $arXMLElement) || array_key_exists($this->mess["IBLOCK_XML2_STORE"], $arXMLElement)) {
            $arElement["STORE_AMOUNT"] = array();
        ...
        $_SESSION["NIKAIMPORT"]["LAST_ID"] = $arElement["ID"];
        return $arElement["ID"];
    }
    ...
    function ImportElementPrices($arXMLElement, &$counter, $arParent = false) {
        ...
        if (isset($arXMLElement[$this->mess["IBLOCK_XML2_STORE_AMOUNT_LIST"]])) { // КоличествоНаСкладах
            $arElement["STORE_AMOUNT"] = array();
            foreach ($arXMLElement[$this->mess["IBLOCK_XML2_STORE_AMOUNT_LIST"]] as $storeAmount) {
                if (isset($storeAmount[$this->mess["IBLOCK_XML2_STORE_ID"]])) {
                    $storeXMLID = $storeAmount[$this->mess["IBLOCK_XML2_STORE_ID"]];
        ...
        } elseif (isset($arXMLElement[$this->mess["IBLOCK_XML2_RESTS"]])) { // Остатки
            $arElement["STORE_AMOUNT"] = array();
    ...
    function SetProductPrice($PRODUCT_ID, $arPrices, $arDiscounts = false) {
        ...
        foreach ($arPrices as $price) {
            if (trim($price[$this->mess["IBLOCK_XML2_PRICE_TYPE_ID"]]) == trim($this->mess["NIKAIMPORT_PRICE_TYPE"])) {
                CPrice::SetBasePrice(
                    $PRODUCT_ID,
                    intval($price[$this->mess["IBLOCK_XML2_PRICE_FOR_ONE"]]),
                    $price[$this->mess["IBLOCK_XML2_CURRENCY"]]
                );
            }
        }
    }
    ...
    function ImportSection($xml_tree_id, $IBLOCK_ID, $parent_section_id) {
        ...
        while ($arS = $rsS->Fetch()) {
            ...
            } elseif ($arS["NAME"] == $this->mess["IBLOCK_XML2_BX_DETAIL_PICTURE"]) { // БитриксКартинкаДетальная
                if (strlen($arS["VALUE"]) > 0) {
            ...
            } elseif ($arS["NAME"] == $this->mess["IBLOCK_XML2_STATUS"]) { // Статус
                $deletedStatus = $arS["VALUE"] === $this->mess["IBLOCK_XML2_DELETED"]; // Удален
        ...
        if ($arSection["ID"]) {
            ...
            $_SESSION["NIKAIMPORT"]["SECTIONS_UPLOAD"][] = $arSection["ID"];
        }
    ...
    function DeletedProducts($start_time, $interval) {
        $counter = array(
            "DEL" => 0,
            "DEA" => 0,
            "NON" => 0,
    ...
    function DeactivateElements($start_time, $interval, $iblock_id = false) {
        $counter = array(
            "DEL" => 0,
            "DEA" => 0,
            "NON" => 0,
        );
        if (!$iblock_id && array_key_exists("bUpdateOnly", $this->next_step) && $this->next_step["bUpdateOnly"]) {
            return $counter;
        }
        $IBLOCK_ID = intval($this->next_step["IBLOCK_ID"]);
        if ($iblock_id) {
            $this->next_step["LAST_ID"] = $_SESSION["NIKAIMPORT"]["DELETED"]["ID"];
            $IBLOCK_ID = $iblock_id;
        }
        if ($IBLOCK_ID < 1) {
            return $counter;
        ...
        if (isset($_SESSION["NIKAIMPORT"]["SECTIONS_UPLOAD"])) {
            $sections = array_unique($_SESSION["NIKAIMPORT"]["SECTIONS_UPLOAD"]);
            $sections = array_values($sections);
            $filters["IBLOCK_SECTION_ID"] = $sections;
        }
        $rsElementIBlock = CIBlockElement::GetList(
            array(
                "ID" => "asc",
            ),
            $filters,
            ...
            array(
                ...
                "TIMESTAMP_X_UNIX",
                "DETAIL_PICTURE",
                "PROPERTY_SOSTAV_GLAVNOGO_V_GRUPPE_PO_MODULNOSTI",
                "PROPERTY_GRUPPA_PO_MODULNOSTI",
                "PROPERTY_DOPOLNITELNYY_TOVAR",
                "PROPERTY_GRUPPA_PO_TSVETU",
                "PROPERTY_GRUPPA_PO_RAZMERU",
                "PROPERTY_GLAVNYY_V_GRUPPE_PO_TSVETU",
                "PROPERTY_RAZMER_SPALNOGO_MESTA",
                "PROPERTY_SKRYT_V_SPISKE_KATALOGA",
            )
        );
        if (!$_SESSION["NIKAIMPORT"]["DELETED"]["ALL"]) {
            $_SESSION["NIKAIMPORT"]["DELETED"]["ALL"] = $rsElementIBlock->SelectedRowsCount();
        }
        $this->mess = IncludeModuleLangFile(__FILE__, "ru", true);
        while ($arElementIBlock = $rsElementIBlock->Fetch()) {
            if (isset($sections) && $arElementIBlock["TIMESTAMP_X_UNIX"] < $_SESSION["NIKAIMPORT"]["START_IMPORT"]) {
                CIBlockElement::Delete($arElementIBlock["ID"]);
                $counter["DEL"]++;
            } else {
                if (empty($arElementIBlock["PROPERTY_SKRYT_V_SPISKE_KATALOGA_VALUE"])) {
                    $hide   = false;
                    $sostav = !empty($arElementIBlock["PROPERTY_SOSTAV_GLAVNOGO_V_GRUPPE_PO_MODULNOSTI_VALUE"]) ? $arElementIBlock["PROPERTY_SOSTAV_GLAVNOGO_V_GRUPPE_PO_MODULNOSTI_VALUE"] : false;
                    if ($sostav && $sostav_uns = @unserialize($arElementIBlock["PROPERTY_SOSTAV_GLAVNOGO_V_GRUPPE_PO_MODULNOSTI_VALUE"])) {
                        if (strlen($sostav_uns["TEXT"]) > 5) {
                            $sostav = $sostav_uns["TEXT"];
                        } else {
                            $sostav = false;
                        }
                    }
                    $gr_razmer = $arElementIBlock["PROPERTY_GRUPPA_PO_RAZMERU_VALUE"];
                    $mattress  = (!empty($gr_razmer) && !empty($arElementIBlock["PROPERTY_RAZMER_SPALNOGO_MESTA_VALUE"])) ? intval(str_replace(array(" ", "x", "х"), "", $arElementIBlock["PROPERTY_RAZMER_SPALNOGO_MESTA_VALUE"])) : false;
                    /**
                     * если нет картинок
                     */
                    if (empty($arElementIBlock["PREVIEW_PICTURE"]) && empty($arElementIBlock["DETAIL_PICTURE"])) {
                        $hide = true;
                    }
                    /**
                     * матрас
                     */
                    elseif ($mattress) {
                        if (!isset($_SESSION["NIKAIMPORT"]["MATTRESS"][$gr_razmer]) && $mattress) {
                            $_SESSION["NIKAIMPORT"]["MATTRESS"][$gr_razmer] = array(
                                "LAST_ID" => $arElementIBlock["ID"],
                                "SIZE"    => $mattress
                            );
                            $hide = false;
                        } elseif ($mattress > $_SESSION["NIKAIMPORT"]["MATTRESS"][$gr_razmer]["SIZE"]) {
                            CIBlockElement::SetPropertyValuesEx(
                                $_SESSION["NIKAIMPORT"]["MATTRESS"][$gr_razmer]["LAST_ID"],
                                $arElementIBlock["IBLOCK_ID"],
                                array(
                                    "SKRYT_V_SPISKE_KATALOGA" => $this->mess["YES"]
                                )
                            );
                            $_SESSION["NIKAIMPORT"]["MATTRESS"][$gr_razmer] = array(
                                "LAST_ID" => $arElementIBlock["ID"],
                            ...
                            $hide = false;
                        } elseif ($_SESSION["NIKAIMPORT"]["MATTRESS"][$gr_razmer]["SIZE"] > $mattress) {
                            $hide = true;
                        }
                    }
                    /**
                     * если есть дополнительный товар скрываем дочерние
                     */
                    elseif (!empty($arElementIBlock["PROPERTY_DOPOLNITELNYY_TOVAR_VALUE"])) {
                        $ids = getArrayString(
                            $arElementIBlock["IBLOCK_ID"],
                            $arElementIBlock["PROPERTY_DOPOLNITELNYY_TOVAR_VALUE"],
                            true
                        );
                        foreach ($ids as $id) {
                            CIBlockElement::SetPropertyValuesEx(
                    ...
                    /**
                     * если есть составные товары скрываем дочерние
                     */
                    elseif ($sostav) {
                        $ids = getArrayString(
                            $arElementIBlock["IBLOCK_ID"],
                    ...
                    /**
                     * если состоит в группе по модульности и не является родителем, скрываем
                     */
                    if (!$hide && !$sostav && !empty($arElementIBlock["PROPERTY_GRUPPA_PO_MODULNOSTI_VALUE"])) {
                        $hide = true;
                    }
                    /**
                     * если состоит в группе по модульности и является родителем, скрываем
                     */
                    if (!$hide && !empty($arElementIBlock["PROPERTY_GRUPPA_PO_TSVETU_VALUE"]) && empty($arElementIBlock["PROPERTY_GLAVNYY_V_GRUPPE_PO_TSVETU_VALUE"])) {
                        $hide = true;
                    }
                    /**
                     * если цена 0
                     */
                    if (!$hide) {
                        $price = getBasePriceProducts($arElementIBlock["IBLOCK_ID"], $arElementIBlock["ID"], true);
                        if ($price["PRICE"] <= 0) {
                            $hide = true;
                        }
                        unset($price);
            ...
            $this->next_step["LAST_ID"] = $arElementIBlock["ID"];
            if ($iblock_id) {
                $_SESSION["NIKAIMPORT"]["DELETED"]["ID"] = $arElementIBlock["ID"];
            }
            unset($arElementIBlock);
            if ($interval > 0 && (time() - $start_time) > $interval) {
                break;
            }
        }
    ...
    function IndexedElements($start_time, $interval) {
        $counter = array(
            "DONE"  => 0,
            "TOTAL" => 0,
            "DEL"   => 0,
        ...
        $rsElementIBlock = CIBlockElement::GetList(
            ...
            array(
                ">ID"                   => $_SESSION["NIKAIMPORT"]["INDEX"]["LAST_ID"],
                "IBLOCK_ID"             => $_SESSION["NIKAIMPORT"]["PARAMS"]["IBLOCK_ID_CATALOG"],
                "ACTIVE"                => "Y",
                "!=PROPERTY_TOVAR_NIKA" => false
            ),
        ...
        if (!$_SESSION["NIKAIMPORT"]["INDEX"]["TOTAL"]) {
            $_SESSION["NIKAIMPORT"]["INDEX"]["TOTAL"] = $counter["TOTAL"] = $rsElementIBlock->SelectedRowsCount();
        }
        CModule::IncludeModule("search");
        while ($arElementIBlock = $rsElementIBlock->GetNext()) {
            if (empty($arElementIBlock["PROPERTY_SKRYT_V_SPISKE_KATALOGA_VALUE"])) {
                CSearch::Index(
                    "iblock",
                    $arElementIBlock["ID"],
                    array(
                        "TITLE"       => $arElementIBlock["NAME"],
                        "PARAM1"      => $arElementIBlock["IBLOCK_TYPE_ID"],
                        "PARAM2"      => $arElementIBlock["IBLOCK_ID"],
                        "BODY"        => $arElementIBlock["DETAIL_TEXT"],
                        "TAGS"        => $arElementIBlock["TAGS"],
                        "SITE_ID"     => $arElementIBlock["LID"],
                        "URL"         => $arElementIBlock["DETAIL_PAGE_URL"],
                        "PERMISSIONS" => array(2),
                    ),
                    true,
                    $bOverWrite
            ...
            $counter["DONE"]++;
            $_SESSION["NIKAIMPORT"]["INDEX"]["LAST_ID"] = $arElementIBlock["ID"];
            unset($arElementIBlock);
            if ($interval > 0 && (time() - $start_time) > $interval) {
                break;
            }
